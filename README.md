...
"喂！星"是一个关于"人人都能拥有一颗私人卫星"的交互式产品原型。用户拥有一颗运行在近地轨道上的虚拟私人卫星，可以用自然语言指挥它拍照、反射阳光、与 AI 对话，甚至用鸿蒙手机实时遥控它的姿态。

距离真正把一颗卫星送上天，我们连 1% 都还没做到。但我们相信这件事终有一天会实现，所以先从一个 Demo 开始。
...
- **星核（Orb Core）**：Web 端主控平台，承担所有调度管理工作，是用户与卫星之间的中枢。用户通过星核下达任务、查看遥测。
...
太空算力、对地观测、太阳光反射这三件事，每一件都已经在现实中被验证过，但它们从来没有被整合在一起，也没有被交到普通人手里。我们想试试看，能不能用"自然语言 + 手机遥控"的方式，让普通人也能摸到太空的边。

**大家都在落地，而我们在上天，参加黑客松的意义就是头脑风暴做一件完全不可能的事。**
...
技术的拼图正在一块一块拼凑。至于什么时候能真正拼完，我们不知道，但觉得很有趣。
...
| 成员 | 背景 | 方向 | 小红书 |
|------|------|------|------|
| 邵瑞琪 | 北大软件微电子学院 | 文艺工医跨界混子 | @SoulRich|
| 曹阳 | 北大直博，AI4S | 北大太空材料学博士 | @擎光 |
| 朱利戈 | 全栈开发，开源作者 | 营收过亿创业者 |@朱利戈 |
| 王宇 | 科技博主，开源作者 | 社恐极客 OPC | @AgentOPC|
...
1. **个人订阅**：基础版免费体验虚拟卫星，Pro 版接入卫星数据与优先任务队列。
...
**主控台**：全屏三维地球、轨道上运行的卫星、开场动画、AI 对话窗口、实时遥测面板、任务照片查看器。

![主控台](docs/images/web-dashboard.png)

**卫星控制台**：2x2 工作台，集成实时遥测面板、可交互的 3D 姿态控制、卫星视角直播画面、地面轨迹投影。

![卫星控制台](docs/images/web-satellite-control.png)

![任务仪表盘](docs/images/mission-dashboard.png)

**卫星追踪**：面向普通人的"当前天空地图"。

![卫星追踪](docs/images/web-tracker.png)

**第一视角**：站在卫星上向外看的沉浸视角，可在地球、月球、太阳、深空四个目标间自由切换，配备 HUD、扫描线、瞄准十字和实时姿态数据。

![第一视角](docs/images/web-pov.png)
...
一个跑在 HarmonyOS 手机上的卫星遥控终端，通过 **WebSocket** 与 Web 端实时通信，让用户把手机当成真正的地面站使用。项目中使用了鸿蒙系统的**陀螺仪**、**AR Engine** 和**小艺（智能语音助手）**能力。

- **一次扫码连接**：手机扫描 Web 端二维码，通过 WebSocket 即刻与卫星配对
- **姿态遥控**：基于鸿蒙陀螺仪传感器，手机怎么转，卫星就怎么转，实时映射到卫星的 Pitch / Yaw / Roll
- **AR 卫星查看器**：基于鸿蒙 AR Engine，通过手机摄像头把卫星 3D 模型"放"到桌面上，可以走近、绕到后面、从下面抬头看
- **小艺语音交互**：接入鸿蒙小艺能力，支持语音下达卫星任务指令
- **3D 预览卡片**：手机上实时显示卫星当前姿态，与 Web 端通过 WebSocket 同步

![鸿蒙遥控终端](docs/images/harmony-remote.png)

![鸿蒙 AR 卫星查看器](docs/images/harmony-ar.png)

### 卫星经济系统

太阳能发电产生算力积分（Energy Points），执行任务时消耗。
...
| 阶段 | 内容 |
|------|------|
| 第一阶段（当前） | 控制系统执行拍照任务，鸿蒙 APP 控制卫星姿态 |
...
![RED HACKATHON 黑客松巅峰赛全体合影](docs/images/hackathon-group-photo.png)

---

**参赛赛事**：黑客松巅峰赛 #Redhackathon 
